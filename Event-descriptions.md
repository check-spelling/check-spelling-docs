...
- [unsupported-configuration](#unsupported-configuration)
  ...
  - [`use_sarif` and `security-events: write`](#usesarif-and-security-events-write)
    - [Resolution](#resolution-13)
  - [`use_sarif` and private repositories](#usesarif-and-private-repositories)
    - [Resolution](#resolution-14)
  - [`use_sarif` and `act`](#usesarif-and-act)
...
This is generated when a word appears more than [unknown_word_limit](Configuration.md#unknownwordlimit) times.
...
If you have enabled [Check filenames and paths](./Feature:-Check-filenames-and-paths) with [`check_file_names: 1`](Configuration.md#checkfilenames), then in addition to checking the contents of files, check-spelling will check the file paths.
...
- If you enabled [Summary Table](./Feature:-Summary-Table) with [`summary_table: 1`](Configuration.md#summarytable) and there is enough space, the individual paths should appear in the details table.
- If you enabled [SARIF output](./Feature:-SARIF-output) with [`use_sarif: 1`](Configuration.md#usesarif) and the repository is public (or is private and has advanced security enabled) and you are a member of the security group / an owner/admin, then individual items should appear in the Sarif report.
...
- Increase [`splitter_timeout`](Feature%3A-Parsing-Timeouts.md#configuration)
...
Check Spelling works by reviewing files and identifying problems with them. You can configure which files it should check by using [excludes](Configuration.md#excludes) and [only](Configuration.md#only)

## Resolution

- Remove entries from [excludes](Configuration.md#excludes)
- Adjust or add items in [only](Configuration.md#only)
...
- Turn off [Check filenames and paths](./Feature:-Check-filenames-and-paths.md).
...
A [Block Ignore](./Feature:-Block-Ignore.md) rule's start pattern matched content in the file, but the corresponding end tag wasn't found.
...
A [Block Ignore](./Feature:-Block-Ignore.md) rule's start pattern matched content in the file, but the corresponding end tag wasn't found. The specific end marker should be listed in the report.
...
The [`suppress_push_for_open_pull_request`](Configuration.md#suppresspushforopenpullrequest) configuration expects there to be an `on:` containing `pull_request` or `pull_request_target` -- if it isn't found, there wouldn't be a place for check-spelling to run.
